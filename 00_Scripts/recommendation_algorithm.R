...
# 2.0 Correlation Analysis - Discretization ----
...
train_corr_tbl <- bake(recipe_obj, newdata = train_readable_tbl)
...
#   Incentivize specialize or promote / Mentorships Role
...
# (Worst Case) Create Personal Development Plan: JobInvolvement, JobSatisfaction, PerfomanceRating
...
tidy(recipe_obj, number = 3) %>%
    filter(str_detect(terms, "YearsAtComp"))
